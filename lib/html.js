...
function getDiffView(currentHtml, originalHtml) {
  ...
  _.forEach(diffPathArr, function (path) {
    ...
    if (dom) {
      dom.type = 'directive';
      let directiveHtml = diffHtmlArr.join('');
      dom.data = directiveHtml.substring(1, directiveHtml.length - 1);
    }
  });
  let diffHtml = $cur.html();
  diffHtml = _.escape(diffHtml)
    .replace(/&lt;JT\-ADD&gt;/g, '<span class="jtDiffAdd">')
    .replace(/&lt;\/JT\-ADD&gt;/g, '</span>')
    .replace(/&lt;JT\-REMOVE&gt;/g, '<span class="jtDiffRemove">')
    .replace(/&lt;\/JT\-REMOVE&gt;/g, '</span>');
  return diffHtml;
}
